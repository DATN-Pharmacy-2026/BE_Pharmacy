...
export class EventRetryPolicyService {
  evaluate(input: RetryPolicyInput): RetryPolicyResult {
    const enabled = `${process.env.EVENT_RETRY_ENABLED ?? 'true'}`.toLowerCase() === 'true';
    const maxAttempts = Number.parseInt(process.env.EVENT_RETRY_MAX_ATTEMPTS ?? '4', 10);
    const delays = parseBackoffDelays(process.env.EVENT_RETRY_BACKOFF_MS);
    const jitterEnabled =
      `${process.env.EVENT_RETRY_JITTER_ENABLED ?? 'true'}`.toLowerCase() === 'true';
    const jitterRatio = Number.parseFloat(process.env.EVENT_RETRY_JITTER_RATIO ?? '0.2');
    ...
    const baseDelay = delays[Math.min(attempt - 1, delays.length - 1)] ?? delays[delays.length - 1] ?? 0;
    ...
    return {
      ...
      reason: !retryable ? 'Non-retryable failure' : exceeded ? 'Retry attempts exhausted' : 'Retry scheduled',
    };
  }
}
