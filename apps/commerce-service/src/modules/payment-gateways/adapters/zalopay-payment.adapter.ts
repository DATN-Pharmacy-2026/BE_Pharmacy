...
export class ZalopayPaymentAdapter implements PaymentGatewayAdapter {
  ...
  async createPayment(input: CreateGatewayPaymentInput): Promise<CreateGatewayPaymentResult> {
    ...
    return { provider: this.provider, providerOrderId: appTransId, requestId: input.requestId, paymentUrl, rawRequest: { data, mac }, rawResponse: { mocked: true } };
  }
  ...
  mapProviderStatus(payload: Record<string, any>): PaymentGatewayTransactionStatus {
    return `${payload.status ?? payload.return_code ?? ''}` === '1' ? PaymentGatewayTransactionStatus.PAID : PaymentGatewayTransactionStatus.FAILED;
  }
  ...
  private verify(payload: Record<string, any>): GatewayVerificationResult {
    ...
    return {
      ...
      providerTransactionId: payload.zp_trans_id ? String(payload.zp_trans_id) : null,
