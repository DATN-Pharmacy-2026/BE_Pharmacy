...
export class NotificationTemplateRendererService {
  ...
  buildContextFromNotificationEvent(
    notificationEvent: Record<string, unknown>,
    explicitContext?: Record<string, unknown>,
  ) {
    const payload =
      notificationEvent.payload && typeof notificationEvent.payload === 'object'
    ...
    return this.prisma.notificationTemplate.findFirst({
      where: {
        ...
        OR: [
          ...
          { locale: DEFAULT_NOTIFICATION_LOCALE, isDefault: true },
        ],
      },
  ...
  private renderTemplate(
    ...
    return {
      ...
      html: template.htmlTemplate
        ? this.interpolateHtml(template.htmlTemplate, context)
        : '',
      template,
    };
  }
